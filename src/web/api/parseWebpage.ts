...
const parseFaviconUrl = (url: string, doc: Document) => {
  ...
  const svgFavicon = doc.querySelector('link[rel="icon"][type="image/svg+xml"], link[rel="apple-touch-icon"][type="image/svg+xml"]');
  if (svgFavicon) {
    return getFaviconUrl(svgFavicon as HTMLLinkElement);
  }

  const pngFavicon = doc.querySelector('link[rel="icon"][type="image/png"], link[rel="apple-touch-icon"][type="image/png"]');
  if (pngFavicon) {
    return getFaviconUrl(pngFavicon as HTMLLinkElement);
  }

  const fallbackFavicon = doc.querySelector('link[rel="icon"], link[rel="shortcut icon"], link[rel="apple-touch-icon"]');
  if (fallbackFavicon) {
